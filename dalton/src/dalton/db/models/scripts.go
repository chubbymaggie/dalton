...
func (script Script) GetCalculatedScore() float64 {

	vec_string := script.GetCVSS_Vector()

	if len(vec_string) > 0 {

		//calculate the base vulnerability scoring based on the vector
		vectors , err := cvss.ParseVectors(vec_string)
		if err != nil {
			return -1
		}

		base := vectors.Score()
		fmt.Println("Score : %d",base)
		return base
	}
